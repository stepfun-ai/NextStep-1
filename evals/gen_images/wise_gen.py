...
class WISEDataset(Dataset):
    ...
    def __getitem__(self, idx):
        name = self.data[idx]["prompt_id"]
        prompt = self.data[idx]["Prompt"]
        return name, prompt


def get_wise_gen_dataloader(
    batch_size: int,
    path: str,
    repeat: int = 1,
...
# WISE inference manager
class WISEInferenceManager(InferenceManager):
    ...
    def run_t2i_benchmark(self, bench_name: str):
        ...
        self.benchmark_dir = os.path.join(self.output_dir, "wise", self.benchmark_name.lower())
        os.makedirs(self.benchmark_dir, exist_ok=True)

        loader = get_wise_gen_dataloader(
...
def get_args():
    ...
    parser.add_argument("--bench_name", type=str, default="WISE_CULTURAL_COMMON_SENSE")
