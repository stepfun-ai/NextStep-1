...
def post_process(result_path: str):
    ...
    for file_name in os.listdir(result_path):
        ...
        new_file_name = os.path.join(result_path, f"{file_name}_{name_set[file_name] - 1}.png")
        os.rename(file_path, new_file_name)


# DPG_BENCH dataset and dataloader
class DPGBenchDataset(Dataset):
    def __init__(self, path: str):
        self.path = path
        self.prompt_files = [f for f in os.listdir(self.path) if f.endswith('.txt')]
    ...
    def __getitem__(self, idx):
        ...
        with open(os.path.join(self.path, prompt_file), 'r', encoding='utf-8') as fp:
            prompt = fp.read()

        return name, prompt


def get_dpg_bench_dataloader(
    batch_size: int,
    path: str,
    repeat: int = 1,
...
# DPG_BENCH inference manager
class DPGBenchInferenceManager(InferenceManager):
    ...
    def run_t2i_benchmark(self, bench_name: str):
        ...
        loader = get_dpg_bench_dataloader(
        ...
        for idx, data in enumerate(loader):
            ...
            for i in range(len(images)):
                images[i].save(f"{self.benchmark_dir}/{names[i]}_rank{self.rank:0>3}.png")
...
def main(args):
    manager = DPGBenchInferenceManager(
...
def get_args():
    ...
    parser.add_argument("--bench_name", type=str, default="DPG_BENCH")
