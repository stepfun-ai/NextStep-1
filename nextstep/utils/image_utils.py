...
def pil_to_bytes(pil_img):
    ...
    format = pil_img.format or 'JPEG'  # If original image has no format info, default to JPEG
...
if __name__ == "__main__":
    ...
    for _mode, path in zip(mode, paths):
        ...
        assert image.min() >= 0 and image.max() <= 255, f"image.min(): {image.min()}, image.max(): {image.max()}, _mode: {_mode}"
